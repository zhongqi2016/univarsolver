...
def psl(prob, sym = True, max_steps = sys.maxsize, epsilon = 1e-2, global_lipschitz_interval = True, known_record = False):
    ...
    :rtype: TestResult
    """
    psp = pslproc.PSLProcessor(rec_v = get_initial_recval(prob, known_record), rec_x = None, problem = prob, eps = epsilon, global_lipint = global_lipschitz_interval, use_symm_lipint = sym)
